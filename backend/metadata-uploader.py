...
def main():
    TEMPLATE_LOCATION = './logstash-template.txt'
    INDEX = 'metadata'
    TYPE = 'mactimes'
    ...
    output = open('/tmp/' + args.case + '.conf', 'w').write(template)
    os.system('cat ' + args.file + ' | /usr/share/logstash/bin/logstash -f /tmp/' + args.case + '.conf')
